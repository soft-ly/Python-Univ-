...
class Sudoku:
    ...
    def __init__(self, width: int = 3, height: Optional[int] = None, board: Optional[Iterable[Iterable[Union[int, None]]]] = None, difficulty: Optional[float] = None, seed: int = randrange(sys.maxsize)):
        ...
        self.__difficulty = difficulty if difficulty is not None else -1
    ...
    def set_difficulty(self, difficulty: float):
        ...
        shuffle(indices)
        self.solve()
        problem_board = [row[:] for row in self.board]
        num_blanks = int(difficulty * self.size * self.size)
        for index in indices[:num_blanks]:
            row, col = divmod(index, self.size)
            problem_board[row][col] = self._empty_cell_value
        self.board = problem_board
        self.__difficulty = difficulty

    def solve(self, raising: bool = False) -> None:
        ...
        if solution:
            ...
            self.board = Sudoku.empty(self.width, self.height).board
